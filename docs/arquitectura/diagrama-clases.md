# Diagrama de Clases

![Diagrama de Clases](./imagenes/DiagramaClases.png)

## Descripción
Este diagrama representa las clases principales del sistema y sus relaciones. Incluye entidades como `Usuario`, `Pacientes`, `Medicamento` y `recordatorios`, además de clases de soporte como `Models`, rutas de atención (`PatientRoute`, `MedicationRoute`, `ReminderRoute`), servicios de `Validaciones` y componentes relacionados con patrones de diseño usados en el proyecto.

## Justificación
El diagrama es acorde con la arquitectura del sistema porque integra la capa de entidades, la lógica de control, los servicios de validación y el acceso a datos. Esto permite reflejar de forma clara cómo se organiza la solución para el registro de pacientes, medicamentos y recordatorios, manteniendo coherencia con la lógica del negocio.

