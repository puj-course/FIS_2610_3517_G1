...
const estilos = `
  ...
  .lt-tarjeta {
    ...
    animation: lt-aparecer .5s ease both;
  }
  @keyframes lt-aparecer {
  ...
  .lt-badge-pendiente {
    background: rgba(251,191,36,.15); color: var(--color-alerta);
    border: 1px solid var(--color-alerta); border-radius: 20px;
  ...
  .lt-badge-atrasado {
    background: rgba(244,114,106,.15); color: var(--color-error);
    border: 1px solid var(--color-error); border-radius: 20px;
    padding: .2rem .7rem; font-size: .78rem; font-weight: 500;
  }
  .lt-boton-tomar {
  ...
  .lt-boton-tomar:disabled {
    background: var(--color-campo); color: var(--color-texto-suave); cursor: not-allowed;
  }
  /* Notificaciones flotantes */
  .lt-notificaciones { position: fixed; top: 1.5rem; right: 1.5rem; z-index: 9999; }
  .lt-notif {
    background: var(--color-tarjeta); border: 1px solid var(--color-exito);
    border-radius: 12px; padding: .9rem 1.2rem;
    display: flex; align-items: center; gap: .75rem;
    box-shadow: 0 8px 32px rgba(0,0,0,.4);
    animation: lt-deslizar .35s ease both;
    font-size: .88rem; margin-bottom: .5rem; color: var(--color-texto);
  }
  .lt-notif.error { border-color: var(--color-error); }
  @keyframes lt-deslizar {
...
const BadgeEstado = ({ estado }) => {
  if (estado === "tomado" || estado === "a_tiempo")
    return <span className="lt-badge-tomado">Tomado</span>;
  if (estado === "atrasado" || estado === "tarde")
    return <span className="lt-badge-atrasado">Atrasado</span>;
...
export default function ListaTomas({ pacienteId, nombrePaciente = "Paciente" }) {
  /*
    Props:
    - pacienteId (número/string): ID del paciente.
      En el HTML original venía de ?pacienteId= en la URL.
    - nombrePaciente (string): nombre para mostrar en el subtítulo.
      En el HTML original venía de ?nombre= en la URL.
    El componente padre (App.jsx / Router) debe obtenerlos del estado
    de sesión o de los parámetros de ruta y pasarlos como props.
  */
  ...
  // ── Cargar tomas del día ────────────────────────────────────────────────
  const cargarTomas = useCallback(async () => {
    setError("");
    setCargando(true);
    if (!pacienteId) {
      setError("No se recibió un paciente seleccionado.");
      setCargando(false);
      return;
    }
    try {
      const res = await fetch(
        `http://localhost:8000/tomas/?paciente_id=${pacienteId}&fecha=${hoy}`
      );
  ...
  const marcarComoTomado = async (medicamentoId, horaProgramada) => {
    ...
    try {
      const res = await fetch("http://localhost:8000/tomas/", {
        method: "POST",
    ...
    <>
      ...
      <div className="lt-body">
        ...
        <div className="lt-tarjeta">
          ...
          <p className="lt-subtitulo">
            Paciente: {nombrePaciente} — {fechaFormateada}
          </p>
