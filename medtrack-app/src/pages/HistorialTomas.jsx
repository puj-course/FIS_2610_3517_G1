...
const CSS = `
  ...
  .ht-root {
    margin: 0; min-height: 100vh; background: #0F1B2D;
  ...
  .ht-marca-icono { width: 36px; height: 36px; background: #2DD4BF; border-radius: 10px; display: flex; align-items: center; justify-content: center; }
  ...
  .ht-btn-volver {
    background: transparent; color: #7A95B0; border: 1.5px solid #243447; border-radius: 10px;
    padding: .5rem 1.1rem; font-size: .85rem; font-family: 'DM Sans', sans-serif;
    cursor: pointer; transition: all .2s; text-decoration: none; display: inline-flex; align-items: center; gap: .4rem;
  }
  ...
  .ht-tarjeta {
    ...
    box-shadow: 0 32px 80px rgba(0,0,0,.5); animation: ht-subir .45s ease both;
  }
  @keyframes ht-subir { from { opacity: 0; transform: translateY(16px); } to { opacity: 1; transform: translateY(0); } }
  ...
  .ht-select {
    ...
    padding: .55rem .9rem; font-size: .88rem; font-family: 'DM Sans', sans-serif;
    transition: border-color .2s; min-width: 260px; outline: none;
  }
  .ht-select:focus { border-color: #2DD4BF; }
  .ht-select option { background: #1E2D3D; }
  .ht-btn-consultar {
    background: #2DD4BF; color: #0F1B2D; border: none; border-radius: 10px;
  ...
  .ht-alerta-error {
    ...
    color: #F4726A; border-radius: 12px; padding: .85rem 1rem; margin-bottom: 1.25rem; font-size: .88rem;
  }
  ...
  .ht-badge { display: inline-flex; align-items: center; border-radius: 20px; padding: .22rem .65rem; font-size: .75rem; font-weight: 600; }
  ...
  .ht-spinner {
    ...
    border-radius: 50%; animation: ht-spin .7s linear infinite; vertical-align: middle; margin-right: .35rem;
  }
...
function BadgeEstado({ estado }) {
  const mapa = {
    tomado: 'ht-badge ht-tomado', a_tiempo: 'ht-badge ht-tomado',
    atrasado: 'ht-badge ht-atrasado', tarde: 'ht-badge ht-atrasado', omitida: 'ht-badge ht-atrasado',
...
export default function HistorialTomas() {
  ...
  async function cargarHistorial() {
    ...
    setError(''); setCargando(true); setHistorial(null);
    try {
      const res = await api.obtenerHistorial(pacienteId);
      if (!res.ok) { setError('Error al consultar el historial.'); return; }
      setHistorial(res.body?.historial ?? []);
    } catch {
    ...
    <>
      ...
      <div className="ht-root">
        <div className="ht-encabezado">
          ...
          <button className="ht-btn-volver" onClick={() => navigate('/dashboard')}>← Volver al dashboard</button>
        </div>

        <div className="ht-tarjeta">
          <h2 className="ht-titulo">Historial de tomas de los últimos 7 días</h2>
          <div className="ht-filtro">
            <select className="ht-select" value={pacienteId} onChange={e => setPacienteId(e.target.value)}>
            ...
            <button className="ht-btn-consultar" onClick={cargarHistorial} disabled={cargando}>
          ...
          <div className="ht-tabla-wrap">
            <table className="ht-tabla">
              <thead>
                <tr>
                  <th>Medicamento</th><th>Fecha</th><th>Hora programada</th><th>Hora tomada</th><th>Estado</th>
                </tr>
              </thead>
              <tbody>
                {historial === null && !cargando && (
                  <tr><td className="ht-vacio" colSpan={5}>Selecciona un paciente para consultar su historial.</td></tr>
                )}
                {cargando && (
                  <tr><td className="ht-vacio" colSpan={5}><span className="ht-spinner"/>Consultando historial…</td></tr>
                )}
                {historial !== null && !cargando && historial.length === 0 && (
                  <tr><td className="ht-vacio" colSpan={5}>No hay historial para este paciente en los últimos 7 días.</td></tr>
                )}
