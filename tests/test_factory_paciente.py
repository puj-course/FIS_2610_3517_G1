"""
test_factory_paciente.py
HU-27: Pruebas del patron Factory para crear pacientes

COMO CORRER ESTAS PRUEBAS:
    pytest tests/test_factory_paciente.py -v

QUE HACE -v?
    "verbose" — muestra el nombre de cada prueba y si paso o fallo.
    Sin -v solo ves puntos verdes o letras rojas.

COMO SE VE UN RESULTADO EXITOSO:
    test_factory_paciente.py::test_fabrica_devuelve_paciente_general   PASSED
    test_factory_paciente.py::test_caso_valido_responde_201            PASSED
    ...
    5 passed in 0.3s

COMO SE VE UN FALLO:
    test_factory_paciente.py::test_caso_valido_responde_201   FAILED
    AssertionError: assert 400 == 201
    (te dice exactamente que recibio 400 pero esperaba 201)
"""
...
# Esto le dice a Python donde encontrar el codigo de tu proyecto.
# __file__ es este archivo, dirname sube una carpeta, abspath
# convierte a ruta absoluta. Sin esto Python no encuentra 'backend'.
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))

# unittest.mock viene incluido con Python, no hay que instalar nada.
# patch = reemplaza temporalmente una funcion con una falsa
# MagicMock = objeto falso que acepta cualquier llamada sin quejarse
...
# ─────────────────────────────────────────────────────────────
# DATOS DE PRUEBA — un diccionario con datos validos de ejemplo
# ─────────────────────────────────────────────────────────────

# Usamos una funcion (no una variable) para que cada prueba
# tenga su PROPIA copia del diccionario. Si fuera variable global
# y una prueba lo modifica, arruina las demas.
...
# ── Explicacion de los mocks ──────────────────────────────────
#
# Tu endpoint hace esto internamente:
#   conn = get_connection()          <- abre SQLite
#   verificar_duplicado(...)         <- consulta la BD
#   cursor.execute(INSERT ...)       <- guarda en BD
#
# En una prueba NO queremos tocar la BD real, entonces:
#   - Reemplazamos get_connection() con una funcion que devuelve
#     un objeto falso (MagicMock) que acepta .cursor(), .commit(),
#     etc. sin hacer nada real.
#   - Reemplazamos verificar_duplicado() para controlar si
#     "ya existe" o no.
#
# La sintaxis 'with patch(...)' hace el reemplazo solo DENTRO
# del bloque with. Al salir, todo vuelve a la normalidad.
#
# "backend.routes.patient_route.get_connection" es la ruta
# completa de la funcion que queremos reemplazar. Debe ser
# donde se usa, no donde se define.
# ─────────────────────────────────────────────────────────────
